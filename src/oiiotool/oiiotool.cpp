...
// --colorinfo
// Print the color-space property info the color config can supply cheaply
// for each named color space (a comma-separated list; an empty list means
// the current top image's color space): one row per field with its
...
#define OTMACTION(act) \
    ...
    ap.arg("--colorinfo %s:COLORSPACES")
      .help("Print the cheaply available color-space property info (image state, color interop ID, encoding, range) for each color space in the comma-separated list, or for the current top image's color space if the list is empty (\"\")")
